...
#### Apprendre les bases de l'analyse multivariée pour révéler des modèles dans les données de composition des communautés

#### Utiliser `R` pour effectuer des ordinations sans contrainte.

#### Choisir les mesures de distance et les transformations appropriées pour effectuer une analyse multivariée.
- Analyse de regroupement
- Analyse en composantes principales (ACP)
- Analyse en coordonnées principales (ACoP)
- Positionnement multidimensionnel non métrique (PMnM)

#### Utiliser `R` pour créer des dendrogrammes
...
Ordination : représentation d'objets (sites, stations, relevés, etc.) sous forme de points le long d'un ou plusieurs axes de référence. Elle répond à la question de savoir comment projeter la dispersion des objets écologiques, qui sont situés dans l'espace multidimensionnel des descripteurs, dans un plus petit nombre de dimensions. 
...
- Manhattan
    ...
    * similaire à Euclidienne, souffre du problème du double zéro
- Corde
    * la distance euclidienne calculée pour une matrice standardisée par ligne
]

.pull-right[
- Hellinger
    * racines carrées des probabilités conditionnelles 
- Chi-carré
    * d[jk] = sum( (x[ij]-x[ik])^2 / (x[ij]+x[ik]) ) / 2 
- Bray-Curtis
    * semimétrique: d[jk] = (sum abs(x[ij]-x[ik]))/(sum (x[ij]+x[ik]))
]

.center[
Où x[ij] x[ik] se réfèrent à la quantité sur l'espèce (colonne) i et les sites (rangées) j & k.
]
...
L'espace des *composants principaux* peut être écrit comme suit :
...
???
Les titres des axes ne sont pas imprimés. Je les ai inclus en haut de chaque graphique, mais ce problème n'est pas résolu.

Le présentateur ou la presentatrice devrait ici mettre l'accent sur la rotation, et parler de ce que sont les scores. Cette compréhension sera utile lorsque les participants utiliseront les fonctions PCA qui sont implémentées dans R.
...
.pull-left[
Cette information peut être obtenue avec la fonction `score()` que nous avons utilisée auparavant :
]
...
Le diagramme de *Shepard* et les valeurs de contraintes peuvent être obtenus par `stressplot()` :
