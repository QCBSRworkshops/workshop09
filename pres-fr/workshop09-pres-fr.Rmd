...
list.of.packages <- c("ape", "gclus", "vegan", "GGally", "PlaneGeometry", "remotes", "matlib", "ade4")
...
1. _Quels sont les effets des précipitations et de la température sur la richesse des espèces ?
2. 2. _Comment l'abondance des microbes change-t-elle entre les hôtes ?
...
L'algèbre matricielle est bien adaptée à l'écologie, car la plupart (si ce n'est la totalité) des _ensembles de données_ avec lesquels nous travaillons sont dans un format _matriciel_.


.pull-left[
Les tableaux de données écologiques sont obtenus sous forme d'observations d'objets ou d'unités d'échantillonnage, et sont souvent enregistrés comme tels :
<br>
...
<br>
où $x_m$ est l'unité d'échantillonnage $m$ ; et $y_n$ est le descripteur écologique qui peut être, _e.g._, l'espèce présente dans une unité d'échantillonnage ou une variable chimique.
]
...
🡐 Analyse **_mode R_** pour les descripteurs ou les espèces

]

.pull-right[
🠅 Analyse **_mode Q_** pour les objets ou les sites
]
]
...
Il a recueilli des données dans **30 localités** le long de la rivière Doubs, qui coule près de la frontière franco-suisse, dans les montagnes du Jura. Il a montré que les communautés de poissons étaient des indicateurs biologiques de ces masses d'eau. 
]
...
Nous pouvons charger leurs données depuis le répertoire `data/` dans cet atelier :
...
---
##### Mesures de (dés)similarité : Coefficients de Jaccard et de Sørensen
...
Ses résultats peuvent être représentés sous forme de dendrogrammes (diagrammes en forme d'arbre), qui décrivent la proximité des observations.
...
> L'algorithme de clustering de Ward ne peut être appliqué qu'avec des distances euclidiennes. Les distances de Bray-Curtis ne sont pas euclidiennes, elles doivent donc être racinées au carré avant d'être utilisées avec l'algorithme de Ward.
...
.pull-right[
Aucun groupe de sites ne peut être défini précisément à partir du biplot NMDS montrant que la plupart des espèces sont présentes dans la plupart des sites, c.-à-d. que quelques sites abritent des communautés spécifiques.
]
