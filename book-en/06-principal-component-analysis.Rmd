...
To achieve this, we first need to standardize the data to have mean zero
and unit variance:
...
where v is the eigenvector and $\lambda$ is the corresponding
eigenvalue.
...
vectors to obtain the principal components:

$$ Y = ZV $$

where Y is the transformed data matrix, V is the matrix of eigenvectors,
and $k$ is the number of principal components.
