...
list.of.packages <- c("ape", "gclus", "vegan", "GGally")
...
Hopefully, this becomes more clear if we represent it!

---

We can transform a square into a parallelogram using a single-axis **shear transformation**. 

For example, if we represent our matrix in the Cartesian place:
...
*We are not torturing you with algebra just for fun!* 

*This understanding is useful to depict some of the applied implications of Eigenvalues and Eigenvectors! *

### Orthogonality

We can rearrange $Av = \lambda v$, so that it becomes...

The covariance matrix of the transformed variables $C = V \Lambda V^T$, gives us:



Remember what a covariance matrix represented as *diagonal matrix* means? Uncorrelated variables!

We have just seen that **Eigenvectors are orthogonal** or uncorrelated! 
...
*We are not torturing you with algebra just for fun*! This understanding is useful to depict some of the applied implications of *Eigenvalues* and *Eigenvectors*! 

### Maximization

We can find a vector $v$

*Include visual explanation of the maximization of variances*


*In a nutshell*, we can obtain new orthogonal vectors that emphasize the variation among observations;
...
.pull-left[
1. Starting point: a matrix of $n$ continuous variables

2\. Standardization

By centering, as in $x' = x_i - \bar{x}$, and

then scaling, as in $x'' = \frac{x_i}{\sigma_x}$. Or, directly:
...
5\. Project the new data onto the feature space
]
...
scaled_features <- as.matrix(scale(varechem))
