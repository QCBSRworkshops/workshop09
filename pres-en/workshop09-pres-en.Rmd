...
[![badge](https://img.shields.io/static/v1?style=for-the-badge&label=Slides&message=09&color=red&logo=html5)](https://qcbsrworkshops.github.io/workshop09/pres-en/workshop09-pres-en.html)
[![badge](https://img.shields.io/static/v1?style=for-the-badge&label=Slides&message=09&color=red&logo=adobe-acrobat-reader)](https://qcbsrworkshops.github.io/workshop09/pres-en/workshop09-pres-en.pdf)
[![badge](https://img.shields.io/static/v1?style=for-the-badge&label=script&message=09&color=2a50b8&logo=r)](https://qcbsrworkshops.github.io/workshop09/book-en/workshop09-script-en.R)
...
####    Choose approprite distance metrics and transformations to perform multivariate analysis
- Clustering analysis
- Principal Component Analysis (PCA)
- Principal Coordinate Analysis (PCoA)
- Non-Metric MultiDimensional Scaling (NMDS)

#### Use `R` to create dendrograms
...
- Ordination: the representation of objects (sites, stations, relevés, etc.) as points along one or several reference axes.

- Reduced space: a space with reduced dimensionality relative to origial dataset.

???

Ordination: the representation of objects (sites, stations, relevés, etc.) as points along one or several reference axes.It addresses the question of how to project the scatter of ecological objects, which are located in the multidimensional space of descriptors, into a smaller number of dimensions. 
...
- For the benefit of analysis, project the scatter of ecological projects from multidimensional space into a smaller number of dimensions of descriptors. 
...
row_sub = apply(spe, 1, 
                function(row) 
                  any(row !=0))
# go through each row, and determine 
# if any value in this row is zero

spe <- spe[row_sub,]
#remove the rows with no species data.   
```
...
- Manhattan
    ...
    * similar to Euclidean, suffer from double zero problem
- Chord
    * the Euclidean distance calculated for a row standardised matrix
]

.pull-right[
- Hellinger
    * square roots of conditional probabilities 
    * performs well in linear ordination
- Chi-square
    * d[jk] = sum( (x[ij]-x[ik])^2 / (x[ij]+x[ik]) ) / 2 
    * used in CA
- Bray-Curtis
    * semimetric: d[jk] = (sum abs(x[ij]-x[ik]))/(sum (x[ij]+x[ik]))
    * used in PCoA and NMDS
]

.center[     

_Where x[ij] x[ik] refer to the quantity on species (column) i and sites (rows) j & k._]
...
--
- Diagnal is zero
- Site 3 and 7 most similar 
- Site 1 and 9 most different
