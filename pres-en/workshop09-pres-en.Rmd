...
.pull-left2[
Throughout this workshop, there will be a series of **challenges** that you can recognize by this rubic cube.
...
.pull-left[
From these many methods we have learned in previous workshops, we can recall the:
...
1. `glm()` with serveral `family()` link functions;
2. `glmer()`.
...
3. How _closely-related_ Wookiee local communities are in relation to their _composition_?
...
We will dive into **R** and **Q** analyses, and we will explore:

1. Association coefficients: similarity, distance, and dependence coefficients
...
.center[**But, let us now introduce a real data set.**]
...
He collected data at **30 localities** along the Doubs river, which runs near France-Switzerland border, in the Jura Mountains.
...
.pull.right3[
.center[![:scale 30%](images/DoubsRiver.png)]
.center[![:scale 30%](https://upload.wikimedia.org/wikipedia/commons/c/cc/Doubs_Laissey.jpg)]

<br>
]
.small[Verneaux, J. (1973) _Cours d'eau de Franche-Comté (Massif du Jura). Recherches écologiques sur le réseau hydrographique du Doubs_. Essai de biotypologie. Thèse d'état, Besançon. 1–257.] 
